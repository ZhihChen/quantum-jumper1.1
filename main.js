...
class QuantumJumper {
    ...
    setupEventListeners() {
        ...
        const casualBtn = getElement('casualModeBtn');
        
        if (casualBtn) casualBtn.addEventListener('click', () => this.startMode('casual'));
        
        addListener('ui2Btn', 'click', () => window.location.href = 'index 2.html');
        addListener('ui3Btn', 'click', () => window.location.href = 'index 3.html');
        
        addListener('resumeBtn', 'click', () => this.togglePause());
        addListener('restartBtn', 'click', () => this.restartGame());
        addListener('quitBtn', 'click', () => this.quitGame());
        addListener('settingsBtn', 'click', () => this.showSettings());
        addListener('closeSettings', 'click', () => this.hideSettings());
        addListener('nextLevelBtn', 'click', () => this.nextLevel());
        
        addListener('restartFromVictoryBtn', 'click', () => this.restartGame());
        addListener('quitFromVictoryBtn', 'click', () => this.returnToModeSelect());
